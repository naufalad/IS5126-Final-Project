...
async def predict(req: PredictRequest):
    try:
        # Import then call model here
        MODEL_DIRECTORY = os.path.join(_DEV_DIR, "models")
        match req.model:
            case 1:
                # BERT + Transformers
                model = joblib.load(os.path.join(MODEL_DIRECTORY, 'chocka.joblib'))["model"]
            case 2:
                # MPNET + XGBoost
                model = joblib.load(os.path.join(MODEL_DIRECTORY, 'habibi.joblib'))["model"]
            case 3:
                # CNN
                model = joblib.load(os.path.join(MODEL_DIRECTORY, 'source.joblib'))["model"]
        # Convert to DataFrame
        input_data = req.subject + " " + req.body
...
async def create(req: EmailRequest):
    try:
        features = extract_email_features(req.body, req.subject or "")
        response = function_calling(features).create_calendar_event()
