...
#     user_input = input("Enter song + artist text (or just artist): ")
#     try:
#         song_info = parse_song_input(user_input)
#         print("Parsed input:", song_info)

#         if song_info["title"]:
#             track = search_spotify_song(song_info["title"], song_info["artist"])
#             if track:
#                 # Add descriptions
#                 track["artist_description"] = get_artist_description(track["artist_id"])
#                 track["song_description"] = get_song_description(track["name"], track["artist"])
...
#         else:
#             print(f"\nℹ️ No song specified. Showing latest songs by {song_info['artist']}:")
#             latest = latest_songs_by_artist(song_info["artist"], limit=5)
#             for i, t in enumerate(latest, 1):
#                 print(f"\n{i}. {t['name']} ({t['release_date']}) - {t['spotify_url']}")
